...
def transition_model(corpus, page, damping_factor):
    ...
    link_prob = damping_factor / len(corpus[page]) if corpus[page] else 0

    for p in corpus:
        model[p] = (1 - damping_factor) / num_pages + link_prob if p in corpus[page] else (1 - damping_factor) / num_pages
    ...
    while True:
        ...
        for page in corpus:
            new_rank = (1 - damping_factor) / num_pages + damping_factor * sum(
                page_rank[i] / len(corpus[i]) if len(corpus[i]) > 0 else page_rank[i] / num_pages
                for i in corpus if page in corpus[i]
            )
